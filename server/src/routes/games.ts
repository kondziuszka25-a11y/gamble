...
router.get('/live-wins', async (_req: Request, res: Response): Promise<void> => {
...
router.post('/play', requireAuth, async (req: Request, res: Response): Promise<void> => {
  ...
  if (user.coins < betAmount) { res.status(400).json({ error: 'Niewystarczające saldo monet' }); return; }
